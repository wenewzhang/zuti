...
fn extract_and_validate_token(req: &HttpRequest) -> Result<Claims, HttpResponse> {
    let auth_header = req
        .headers()
        .get("Authorization")
...
#[get("/get_disks")]
async fn get_disks(req: HttpRequest) -> impl Responder {
    // 1. 验证 JWT token
